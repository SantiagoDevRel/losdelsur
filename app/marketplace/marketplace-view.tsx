...
// app/marketplace/marketplace-view.tsx
// Vista interactiva del marketplace mock. Maneja el estado del filtro
// por categoría y filtra la lista en cliente. Sigue siendo data
// estática (sin DB), solo se hizo clickeable para validar la UX.
...
type Categoria = "Salud" | "Arte" | "Servicios" | "Educación" | "Legal";
...
const CATEGORIAS = ["Todos", "Salud", "Arte", "Servicios", "Educación", "Legal"] as const;
type CategoriaChip = (typeof CATEGORIAS)[number];

export default function MarketplaceView() {
  const [activa, setActiva] = useState<CategoriaChip>("Todos");

  const visibles =
    activa === "Todos"
      ? PROFILES
      : PROFILES.filter((p) => p.categoria === activa);

  return (
    <main className="mx-auto min-h-dvh w-full max-w-3xl pb-[110px] pt-14 sm:pt-20">
      <header className="px-5 pb-4 sm:px-8 sm:pb-6">
        <div className="eyebrow">
          {visibles.length} HINCHAS · OFRECIENDO SERVICIOS
        </div>
        <h1
      ...
      {/* Chips de categoría — clickeables, filtran la lista en cliente */}
        ...
        {CATEGORIAS.map((cat) => {
          ...
          return (
            <button
              ...
              className="shrink-0 border px-3.5 py-1.5 text-[11px] font-bold uppercase tracking-[0.08em] transition-colors active:scale-95"
              style={{
                borderColor: isActive
                  ? "var(--color-verde-neon)"
                  : "rgba(255,255,255,0.15)",
                color: isActive ? "var(--color-verde-neon)" : "#a8a8a3",
                background: isActive
                  ? "rgba(43,255,127,0.08)"
                  : "transparent",
                borderRadius: 999,
              }}
            >
      ...
      {/* Lista vertical de perfiles filtrada */}
      {visibles.length === 0 ? (
        <p className="mt-8 px-5 text-center text-[12px] uppercase tracking-[0.1em] text-white/40 sm:px-8">
          Sin perfiles en esta categoría todavía
        </p>
      ) : (
